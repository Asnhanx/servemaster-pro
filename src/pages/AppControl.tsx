    ...
    <div className="bg-background-dark text-text-light font-sans pt-16">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="flex flex-col lg:flex-row items-center gap-16">
            ...
            >
              ...
              <h1 className="text-5xl md:text-7xl lg:text-8xl font-display font-black tracking-tighter mb-6 leading-tight whitespace-pre-line">
                {t.app.heroTitle.split('\n')[0]}<br />
                <span className="text-gradient">{t.app.heroTitle.split('\n')[1]}</span>
              </h1>
              ...
              <div className="flex flex-col sm:flex-row justify-center lg:justify-start items-center space-y-4 sm:space-y-0 sm:space-x-6">
                <button className="w-full sm:w-auto flex items-center justify-center space-x-3 bg-white text-black px-8 py-4 rounded-2xl text-lg font-bold hover:bg-gray-200 transition-all transform hover:scale-105">
                  ...
                  <div className="text-left leading-tight">
                    <div className="text-[10px] font-normal">{t.app.downloadIosLabel}</div>
                    <div>{t.app.downloadIos}</div>
                  </div>
                </button>
                <button className="w-full sm:w-auto flex items-center justify-center space-x-3 bg-white text-black px-8 py-4 rounded-2xl text-lg font-bold hover:bg-gray-200 transition-all transform hover:scale-105">
                  <span className="material-symbols-outlined text-3xl">android</span>
                  <div className="text-left leading-tight">
                    <div className="text-[10px] font-normal">{t.app.downloadAndroidLabel}</div>
                    <div>{t.app.downloadAndroid}</div>
                  </div>
                </button>
              </div>
            </motion.div>
              ...
              <div className="relative w-[280px] h-[570px] sm:w-[320px] sm:h-[650px] bg-black rounded-[3rem] border-[12px] border-surface-dark shadow-2xl overflow-hidden">
                ...
                <div className="absolute inset-0 bg-[#0f0f0f] flex flex-col pt-12 pb-8 px-6">
                  <div className="flex justify-between items-center mb-8">
                    <div>
                      <h3 className="text-white font-bold text-xl">{t.app.mockupDrillMode}</h3>
                      <p className="text-primary text-xs flex items-center mt-1">
                        <span className="w-2 h-2 rounded-full bg-primary mr-2 animate-pulse"></span>
                        {t.app.mockupConnected}
                      </p>
                    </div>
      ...
      <section className="py-16 md:py-24 bg-surface-dark relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 md:mb-20">
            <h2 className="text-3xl md:text-5xl font-display font-bold mb-6">{t.app.featuresTitle}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: 'tune', title: t.app.feat1Title, desc: t.app.feat1Desc },
              { icon: 'route', title: t.app.feat2Title, desc: t.app.feat2Desc },
              { icon: 'query_stats', title: t.app.feat3Title, desc: t.app.feat3Desc },
              { icon: 'cloud_download', title: t.app.feat4Title, desc: t.app.feat4Desc },
              { icon: 'watch', title: t.app.feat5Title, desc: t.app.feat5Desc },
              { icon: 'group', title: t.app.feat6Title, desc: t.app.feat6Desc },
            ].map((feat, i) => (
              <div key={i} className="bg-background-dark p-6 md:p-8 rounded-3xl border border-border-dark hover:border-primary/50 transition-colors group">
                <div className="w-14 h-14 rounded-2xl bg-surface-dark border border-border-dark flex items-center justify-center mb-6 group-hover:bg-primary/10 group-hover:border-primary/30 transition-colors">
                  <span className="material-symbols-outlined text-2xl text-white group-hover:text-primary transition-colors">{feat.icon}</span>
                </div>
      ...
      <section className="py-16 md:py-24 relative overflow-hidden text-center bg-background-dark">
        <div className="max-w-3xl mx-auto px-4 relative z-10">
          <h2 className="text-3xl md:text-5xl font-display font-bold mb-6">{t.app.ctaTitle}</h2>
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-6">
            ...
            <div className="flex flex-col justify-center text-left">
              <p className="font-bold mb-2">{t.app.scanQr}</p>
              <p className="text-sm text-text-muted">iOS 14.0+<br />Android 9.0+</p>
            </div>
          </div>
        </div>
      </section>
